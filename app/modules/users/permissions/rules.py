...
class WorkspaceMemberRule(ActiveUserRoleRule):
    """
    Ensure that the current_user is a member of a workspace which the given
    object is associated with.
    """

    def __init__(self, obj, **kwargs):
        super(WorkspaceMemberRule, self).__init__(**kwargs)
        self._obj = obj

    def check(self):
        if not hasattr(self._obj, 'check_workspace_member'):
            return False
        return self._obj.check_workspace_member(current_user) is True
