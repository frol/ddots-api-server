...
from .permissions import OwnerRolePermission
...
class OwnerMixin(object):
    """
    Common mixin to provide relationship with creator (User) and method to sheck owner

    __owner_backref_name__: str - is used to provide backref verbose_name
    """
    query_class = OwnerRolePermission.get_query_class()
    __owner_backref_name__ = None

    @declared_attr
    def creator_id(cls):
        return db.Column('creator_id', db.Integer, db.ForeignKey('user.id'), nullable=False)

    @declared_attr
    def creator(cls):
        return db.relationship(
            User,
            backref=db.backref(cls.__owner_backref_name__, cascade='delete, delete-orphan')
        )
